...
TC_RE_pro=csvread('Output_Cost.csv');%B CNY
...
%sum(TC_ideal)
(sum(TC_max_LAND)+sum(TC_max_SEA))./sum(D_wc).*10^6./(sum(TC_ideal).*10^6./sum(D_county)).*100 % %
(sum(TC_min_LAND)+sum(TC_min_SEA))./sum(D_wc).*10^6./(sum(TC_ideal).*10^6./sum(D_county)).*100 % %
